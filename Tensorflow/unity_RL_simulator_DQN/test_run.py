...
w0 = 1000         #충돌시
w1 = 100000       #goal도착
w2 = 250           #차와 goal간 방향이 잘 맞는 정도에 비례한 reward
w2Exp = 2
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_episodes):
            ...
            state, _vecLen, _headingDiff, _done = dyrosRLdata.getStep()
            #reward 초기화
            reward_w = []
            ...
            while not done:#1: 충돌, 0: 비충돌
                ...
                next_state, vecLen, headingDiff, done = dyrosRLdata.getStep()
                reward = 1
                #print("--------- vecLen : ", vecLen)
                ...
                if done == 1:
                    ...
                    reward_w[1] = 0
                    #headingDiff -> 0 ~ 100
                    reward_w[3] = pow(w3 / (vecLen + 1), w3Exp) #1 + (pow((127 - vecLen), w3Exp)       * pow(w3, w3Exp)) #far -> 127 / near(goal) -> 0
                    if w1 < reward_w[3]:
                        reward_w[3] = w1
                    reward_w[2] = pow(w2 / (headingDiff + 1), w2Exp)
                    if w1 < reward_w[2]:
                        reward_w[2] = w1
...
#                   filename = mainDQN.saver.save(sess, checkpoint_name)
            ...
            print("Episode: {} steps: {} done: {} reward: {}".format(episode, step_count, done, reward))
